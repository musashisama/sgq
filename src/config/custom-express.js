...
app.use('/*', function (req, resp, next) {
  if (req.baseUrl != '') {
    ...
    req.baseUrl == '/login' ? registro.corpo = null : registro.corpo = req.body
    registro.metodo = req.method;
    registro.timestamp = new Date().toISOString();
    req.isAuthenticated() ? registro['usuarioLogado'] = req.user.cpf : registro['usuarioLogado'] = 'Não Logado';
    const requestIp = require('request-ip');
    registro['clientIP'] = requestIp.getClientIp(req);
    const baseDao = new BaseDao(conn);   
    baseDao.logger(registro)
    .then(()=>{return;})
