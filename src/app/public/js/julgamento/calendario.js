  ...
  $(document).ready(function () {
    ...
    getEventos().then((eventos) => {
      eventos.forEach((evento) => {
        evento.start = moment(evento.start, 'DD/MM/YYYY')
          .tz('America/Sao_Paulo')
          .format();
        evento.end = moment(evento.end, 'DD/MM/YYYY')
          .tz('America/Sao_Paulo')
          .format();
      });
...
function handleCal(reg, metodo) {
  let data = {
    classNames: reg.classNames,
    uniqueId: reg.extendedProps.uniqueId,
    start: moment(reg.start).tz('America/Sao_Paulo').format('DD/MM/YYYY'),
    end: moment(reg.end).tz('America/Sao_Paulo').format('DD/MM/YYYY'),
    allDay: true,
  };
  $.ajax({
